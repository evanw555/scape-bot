...
export async function fetchAllPrivilegedRoles(): Promise<Record<Snowflake, Snowflake>> {
    const client: PGClient = state.getPGClient();
    const result: Record<Snowflake, Snowflake> = {};
    const queryResult = await client.query<{guild_id: Snowflake, role_id: Snowflake}>('SELECT * FROM privileged_roles;');
    for (const row of queryResult.rows) {
...
export async function deletePrivilegedRole(guildId: Snowflake, roleId: Snowflake): Promise<void> {
    const client: PGClient = state.getPGClient();
    await client.query('DELETE FROM privileged_roles WHERE guild_id = $1 AND role_id = $2;', [guildId, roleId]);
}

export async function writePrivilegedRole(guildId: Snowflake, roleId: Snowflake): Promise<void> {
    const client: PGClient = state.getPGClient();
    await client.query('INSERT INTO privileged_roles VALUES ($1, $2) ON CONFLICT (guild_id) DO UPDATE SET role_id = EXCLUDED.role_id;', [guildId, roleId]);
}
